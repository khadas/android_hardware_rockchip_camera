...
#define  Sensor_NEWEST_TUNING_XML "22-May-2014_OUYANG_ar0330_FX288_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "ar0330: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "ar0330: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "ar0330: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "ar0330: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "ar0330: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "ar0330: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x20U                           /**< i2c slave address of the ar0330 camera sensor */
#define Sensor_SLAVE_ADDR2      0x20U
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the ar0330 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "ar0330_MIPI";


extern const IsiRegDescription_t Sensor_g_aRegDescription_twolane[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_2048_1536[];
extern const IsiRegDescription_t Sensor_g_2048x1536P30_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_2048x1536P25_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_2048x1536P20_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_2048x1536P15_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_2048x1536P10_twolane_fpschg[];



/*
extern const IsiRegDescription_t Sensor_g_1536x972P30_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1536x972P25_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1536x972P20_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1536x972P15_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1536x972P10_fourlane_fpschg[];
    ...
    else
    {
        if(mipi_lanes == SUPPORT_MIPI_ONE_LANE){
            ...
            switch (pIsiSensorCaps->Index) 
            {
                case 0:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_2048_1536P30;
                    break;
                }
                case 1:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_2048_1536P25;
                    break;
                }

				case 2:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_2048_1536P20;
                    break;
                }
                case 3:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_2048_1536P15;
                    break;
                }
				case 4:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_2048_1536P10;
                    break;
                }
                default:
                {
...
)
{
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
			...
			case ISI_RES_2048_1536P30:
			case ISI_RES_2048_1536P25:
			case ISI_RES_2048_1536P20:
			case ISI_RES_2048_1536P15:
			case ISI_RES_2048_1536P10:
	        {
				...
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_2048_1536)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane ISI_RES_2048_1536 \n", __FUNCTION__ );
			            }
					}
				if (pConfig->Resolution == ISI_RES_2048_1536P30) {                        
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2048x1536P30_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_2048_1536P25) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2048x1536P25_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_2048_1536P20) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2048x1536P20_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_2048_1536P15) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2048x1536P15_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_2048_1536P10) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2048x1536P10_twolane_fpschg);
                    } 
        		
				}	
	            usLineLengthPck = 0x04DA;
				if (pConfig->Resolution == ISI_RES_2048_1536P30) {
	            	usFrameLengthLines = 0x0528;
				}else if(pConfig->Resolution == ISI_RES_2048_1536P25) {
	            	usFrameLengthLines = 0x0630; 
				}else if(pConfig->Resolution == ISI_RES_2048_1536P20) {
	            	usFrameLengthLines = 0x07BC; 
				}else if(pConfig->Resolution == ISI_RES_2048_1536P15) {
	            	usFrameLengthLines = 0x0A50; 
				}else if(pConfig->Resolution == ISI_RES_2048_1536P10) {
	            	usFrameLengthLines = 0x0F78; 
				}
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 98;
	            break;
	...
	rVtPixClkFreq = 49200000;  //Sensor_get_PCLK(pSensorCtx, xclk);    
...
static RESULT ar0330_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,ar0330_read_i2c(handle,address));
...
static int update_awb_gain(IsiSensorHandle_t   handle,int R_gain, int G_gain, int B_gain)
{
    ...
    if (R_gain>0x400) {
        ar0330_write_i2c(handle,0x5186, R_gain>>8);
        ar0330_write_i2c(handle,0x5187, R_gain & 0x00ff);
    }
    if (G_gain>0x400) {
        ar0330_write_i2c(handle,0x5188, G_gain>>8);
        ar0330_write_i2c(handle,0x5189, G_gain & 0x00ff);
    }
    if (B_gain>0x400) {
        ar0330_write_i2c(handle,0x518a, B_gain>>8);
        ar0330_write_i2c(handle,0x518b, B_gain & 0x00ff);
    }
    return 0;
}
// call this function after ar0330 initialization
...
)
{
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
		...
		TRACE( Sensor_ERROR, "%s: don't support one lane\n", __FUNCTION__ );
	}
	else if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_FOUR_LANE){
        //result = IsiRegDefaultsApply( pSensorCtx, Sensor_g_aRegDescription_fourlane);
        TRACE( Sensor_ERROR, "%s: don't support four lane\n", __FUNCTION__ );
    }
	else if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_TWO_LANE){
...
)
{
    ...
    if (on == BOOL_TRUE)
    {
        ...
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_MODE_SELECT, 0x5c );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    {   
        ...
        result = Sensor_IsiRegReadIss ( pSensorCtx, Sensor_MODE_SELECT, &RegValue);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_MODE_SELECT, 0x58 );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    else
    {
        ...
        if ( !NrOfBytes )
        {
            NrOfBytes = Sensor_I2C_NR_DAT_BYTES;
        }
        //TRACE( Sensor_ERROR, "%s (IsiGetNrDatBytesIss %d 0x%08x)\n", __FUNCTION__, NrOfBytes, address);
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    //TRACE( Sensor_ERROR, "%s (IsiGetNrDatBytesIss %d 0x%08x 0x%08x)\n", __FUNCTION__, NrOfBytes, address, value);
...
)
{
    ...
    usGain = (uint16_t)((sqrt(0.02078*0.02078-4.0f*0.002404*(1.012-NewGain))-0.02078)/(2.0f*0.002404)+0.5);
	...
	if(1)
	   {
		   result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3060, usGain);
		   RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    calc = (float)usGain;
	pSensorCtx->AecCurGain = 0.002404*calc*calc + 0.02078*calc + 1.012;
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
	...
	if( CoarseIntegrationTime != pSensorCtx->OldCoarseIntegrationTime )
	{
		result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3012, CoarseIntegrationTime );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        pSensorCtx->OldCoarseIntegrationTime = CoarseIntegrationTime;   // remember current integration time
        *pNumberOfFramesToSkip = 1U; //skip 1 frame
	}
			...
			case SUPPORT_MIPI_TWO_LANE:
			{
				...
				switch(Resolution)
			    {
					...
					case ISI_RES_2048_1536P10:
						...
						//AFPSCHECKANDADD( ISI_RES_2048_1536P30);
						//AFPSCHECKANDADD( ISI_RES_2048_1536P25);
						//AFPSCHECKANDADD( ISI_RES_2048_1536P20);
						//AFPSCHECKANDADD( ISI_RES_2048_1536P15);
						AFPSCHECKANDADD( ISI_RES_2048_1536P10);
						break;
...
static RESULT Sensor_IsiGetSensorI2cInfo(sensor_i2c_info_t** pdata)
{
    ...
    pSensorI2cInfo->reg_size = Sensor_I2C_NR_ADR_BYTES;
    pSensorI2cInfo->value_size = Sensor_I2C_NR_DAT_BYTES;
