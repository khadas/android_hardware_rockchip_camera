...
#define  Sensor_NEWEST_TUNING_XML "22-May-2014_OUYANG_GC5005_FX288_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "GC5005: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "GC5005: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "GC5005: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "GC5005: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "GC5005: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "GC5005: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x6eU                           /**< i2c slave address of the GC5005 camera sensor */
#define Sensor_SLAVE_ADDR2      0x6eU
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the GC5005 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "GC5005_MIPI";
		  ...
		  case ISI_RES_2592_1944P7:
	        {
	            ...
	            usLineLengthPck = 0x038a;
				if (pConfig->Resolution == ISI_RES_2592_1944P30) {
	            				usFrameLengthLines = 0x07c0; //hkw
				}
				if (pConfig->Resolution == ISI_RES_2592_1944P25) {
	            				usFrameLengthLines = 0x0942; //hkw
				}
				if (pConfig->Resolution == ISI_RES_2592_1944P20) {
	            				usFrameLengthLines = 0x0ba0; //hkw
				}
				if (pConfig->Resolution == ISI_RES_2592_1944P15) {
	            				usFrameLengthLines = 0x0F80; //hkw
				}
				if(pConfig->Resolution == ISI_RES_2592_1944P10) {
	            			usFrameLengthLines = 0x1740; //hkw
				}
				if(pConfig->Resolution == ISI_RES_2592_1944P7) {
	            			usFrameLengthLines = 0x2136; //hkw
				}
				//need mod
				usFrameLengthLines = 0x1740; //hkw
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 900;
	            break;
...
)
{
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		Sensor_IsiRegWriteIss(pSensorCtx,0x21, 0x0f);//gc5005_otp_info.Val21
		Sensor_IsiRegWriteIss(pSensorCtx,0x29, 0x22);//gc5005_otp_info.Val29
		Sensor_IsiRegWriteIss(pSensorCtx,0xd8, 0x0b);//04
	...
	{                                    
		Sensor_IsiRegWriteIss(pSensorCtx,0xfe, 0x00);
		Sensor_IsiRegWriteIss(pSensorCtx,0x21, 0x16);//0a
		Sensor_IsiRegWriteIss(pSensorCtx,0x29, 0x28);//22
		Sensor_IsiRegWriteIss(pSensorCtx,0xd8, 0x12);//04
	...
	{
 		Sensor_IsiRegWriteIss(pSensorCtx,0xfe, 0x00); 
		Sensor_IsiRegWriteIss(pSensorCtx,0x21, 0x16);
		Sensor_IsiRegWriteIss(pSensorCtx,0x29, 0x28);
		Sensor_IsiRegWriteIss(pSensorCtx,0xd8, 0x12);
		//write_cmos_sensor(0xe8, 0x01);
		//write_cmos_sensor(0xec, 0x01);
 	
		//analog gain
		Sensor_IsiRegWriteIss(pSensorCtx,0xb6,  0x02);//
	...
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		//analog gain
		Sensor_IsiRegWriteIss(pSensorCtx,0xb6,  0x03);//
	...
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		//analog gain
		Sensor_IsiRegWriteIss(pSensorCtx,0xb6,  0x04);//
	...
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		//analog gain
		Sensor_IsiRegWriteIss(pSensorCtx,0xb6,  0x05);//
	...
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		//analog gain
		Sensor_IsiRegWriteIss(pSensorCtx,0xb6,  0x06);//
	...
	{                                                                                                                                                                                  
		...
		//analog gain
		Sensor_IsiRegWriteIss(pSensorCtx,0xb6,  0x07);//                                                       
    ...
    *pSetGain = pSensorCtx->AecCurGain;
    ...
    TRACE( Sensor_ERROR, "%s: setgain mubiao(%f) shiji(%f)\n", __FUNCTION__, NewGain, *pSetGain);
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
	...
	if(CoarseIntegrationTime <= 30)
  	{
		Sensor_IsiRegWriteIss(pSensorCtx,0xfe, 0x00);
		Sensor_IsiRegWriteIss(pSensorCtx,0x32, 0xfc);
		Sensor_IsiRegWriteIss(pSensorCtx,0xb0, 0x58);		
  	}
  	else 
	{
		Sensor_IsiRegWriteIss(pSensorCtx,0xfe, 0x00);
		Sensor_IsiRegWriteIss(pSensorCtx,0x32, 0xf8);
		Sensor_IsiRegWriteIss(pSensorCtx,0xb0, 0x50);		
  	}	
	Sensor_IsiRegWriteIss(pSensorCtx,0xfe, 0x00);
	//Sensor_IsiRegWriteIss(pSensorCtx,0x03, 0x20);
	//Sensor_IsiRegWriteIss(pSensorCtx,0x04, 0x00);
