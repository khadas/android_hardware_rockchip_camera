...
static RESULT GS8604_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,GS8604_read_i2c(handle,address));
...
static int update_awb_gain(IsiSensorHandle_t   handle,int R_gain, int G_gain, int B_gain)
{
    ...
    if (R_gain>0x400) {
        GS8604_write_i2c(handle,0x5186, R_gain>>8);
        GS8604_write_i2c(handle,0x5187, R_gain & 0x00ff);
    }
    if (G_gain>0x400) {
        GS8604_write_i2c(handle,0x5188, G_gain>>8);
        GS8604_write_i2c(handle,0x5189, G_gain & 0x00ff);
    }
    if (B_gain>0x400) {
        GS8604_write_i2c(handle,0x518a, B_gain>>8);
        GS8604_write_i2c(handle,0x518b, B_gain & 0x00ff);
    }
    return 0;
}
// call this function after GS8604 initialization
