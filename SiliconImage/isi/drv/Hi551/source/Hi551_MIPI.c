...
#include "Hi551_MIPI_priv.h"

#define  Sensor_NEWEST_TUNING_XML "22-May-2014_OUYANG_Hi551_FX288_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "Hi551: ", INFO,    1U );
CREATE_TRACER( Sensor_WARN , "Hi551: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "Hi551: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "Hi551: ", INFO,     1U );

CREATE_TRACER( Sensor_REG_INFO , "Hi551: ", INFO, 1);
CREATE_TRACER( Sensor_REG_DEBUG, "Hi551: ", INFO, 1U );

#define Sensor_SLAVE_ADDR       0x40U                           /**< i2c slave address of the Hi551 camera sensor */
#define Sensor_SLAVE_ADDR2      0x40U
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the Hi551 integrated AD5820 */
#define Sensor_OTP_SLAVE_ADDR   0x40U
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "Hi551_MIPI";
...
extern const IsiRegDescription_t Sensor_g_1296x972P7_twolane_fpschg[];
...
#define Sensor_I2C_NR_DAT_BYTES     (2U)                        // 16 bit registers
...
static RESULT Sensor_IsiRegWriteIss_1( IsiSensorHandle_t handle, const uint32_t address, const uint32_t value );
...
)
{
	...
	TRACE( Sensor_DEBUG,  "%s:HalDevID(%u) I2cBusNum(%u) SlaveAddress(%x) I2cAfBusNum(%u) SlaveAfAddress(%x) lanes(%u)\n",  __FUNCTION__, pSensorCtx->IsiCtx.HalDevID , pSensorCtx->IsiCtx.I2cBusNum ,pSensorCtx->IsiCtx.SlaveAddress,pSensorCtx->IsiCtx.I2cAfBusNum,pSensorCtx->IsiCtx.SlaveAfAddress ,pSensorCtx->IsiSensorMipiInfo.ucMipiLanes);
    ...
    else
    {
        if(mipi_lanes == SUPPORT_MIPI_ONE_LANE){
            ...
            switch (pIsiSensorCaps->Index) 
            {
				...
				case 11:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_1296_972P7;
                    break;
                }
                default:
                {
...
static int Sensor_get_PCLK( Sensor_Context_t *pSensorCtx, int XVCLK)
{
	 // calculate sysclk
...
static RESULT IsiRegDefaultsApply_1
(
    IsiSensorHandle_t         handle,
...
)
{
    ...
    TRACE( Sensor_INFO, "%s (enter)ucMipiLanes(%d)---pConfig->Resolution:%x\n", __FUNCTION__, pSensorCtx->IsiSensorMipiInfo.ucMipiLanes, pConfig->Resolution);
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
			...
			case ISI_RES_1296_972P7:
	        {
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply_1((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_1296_972)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane ISI_RES_1296_972 \n", __FUNCTION__ );
			            }
					}
				if (pConfig->Resolution == ISI_RES_1296_972P30) {                        
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1296x972P30_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_1296_972P25) {
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1296x972P25_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_1296_972P20) {
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1296x972P20_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_1296_972P15) {
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1296x972P15_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_1296_972P10) {
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1296x972P10_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_1296_972P7) {
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1296x972P7_twolane_fpschg);
                    } 
        		
				}	
	            usLineLengthPck = 0x0b7c;
				if (pConfig->Resolution == ISI_RES_1296_972P30) {
	            	usFrameLengthLines = 0x07bc;
				}else if(pConfig->Resolution == ISI_RES_1296_972P25) {
					usFrameLengthLines = 0x095a;
				}else if(pConfig->Resolution == ISI_RES_1296_972P20) {
					
					usFrameLengthLines = 0x0bb1;
					
				}else if(pConfig->Resolution == ISI_RES_1296_972P15) {
					
					usFrameLengthLines = 0x0f78;
					
				}else if(pConfig->Resolution == ISI_RES_1296_972P10) {
					
					usFrameLengthLines = 0x1734;
				}else if(pConfig->Resolution == ISI_RES_1296_972P7) {
					
					usFrameLengthLines = 0x1f28;
				}
	
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 440;//shadow modify 440 to 420
	        ...
	        case ISI_RES_2592_1944P7:
	        {
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply_1((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_2592_1944)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane case ISI_RES_2592_1944 \n", __FUNCTION__ );
			            }
					}
				if (pConfig->Resolution == ISI_RES_2592_1944P30) {
						TRACE( Sensor_ERROR, "%s: ------ISI_RES_2592_1944P30----- \n", __FUNCTION__ );
						result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2592x1944P30_twolane_fpschg);
                    } else if (pConfig->Resolution == ISI_RES_2592_1944P25) {
                    	TRACE( Sensor_ERROR, "%s: ------ISI_RES_2592_1944P25----- \n", __FUNCTION__ );
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2592x1944P25_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_2592_1944P20) {
                    	TRACE( Sensor_ERROR, "%s: ------ISI_RES_2592_1944P20----- \n", __FUNCTION__ );
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2592x1944P20_twolane_fpschg);
                    } else if (pConfig->Resolution == ISI_RES_2592_1944P15) {
                    	TRACE( Sensor_ERROR, "%s: ------ISI_RES_2592_1944P15----- \n", __FUNCTION__ );
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2592x1944P15_twolane_fpschg);
                    } else if (pConfig->Resolution == ISI_RES_2592_1944P10) {
                    	TRACE( Sensor_ERROR, "%s: ------ISI_RES_2592_1944P10----- \n", __FUNCTION__ );
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2592x1944P10_twolane_fpschg);
                    } else if (pConfig->Resolution == ISI_RES_2592_1944P7) {
                    	TRACE( Sensor_ERROR, "%s: ------ISI_RES_2592_1944P7----- \n", __FUNCTION__ );
                        result = IsiRegDefaultsApply_1( (IsiSensorHandle_t)pSensorCtx, Sensor_g_2592x1944P7_twolane_fpschg);
                    } 
				}
	            usLineLengthPck = 0x0b7c;
				if (pConfig->Resolution == ISI_RES_2592_1944P30) {
	            	usFrameLengthLines = 0x07ca;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P25) {
					usFrameLengthLines = 0x0958;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P20) {
					usFrameLengthLines = 0x0baf;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P15) {
					usFrameLengthLines = 0x0f94;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P10) {
					usFrameLengthLines = 0x175e;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P7) {
					usFrameLengthLines = 0x1f28;
				}

				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 880;
...
)
{
    ...
    TRACE( Sensor_DEBUG, "%s%s (enter)  Res: 0x%x  0x%x\n", __FUNCTION__, pSensorCtx->isAfpsRun?"(AFPS)":"",
...
)
{
   ...
   // result = Sensor_IsiRegWriteIss_1 ( pSensorCtx, Sensor_SOFTWARE_RST, 0x01U );
   // RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    result = Sensor_IsiRegWriteIss_1( pSensorCtx, Sensor_MODE_SELECT, 0x00 );
    if ( result != RET_SUCCESS )
    {
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
		result = IsiRegDefaultsApply_1( pSensorCtx, Sensor_g_aRegDescription_onelane);
	}
	else if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_TWO_LANE){
		TRACE( Sensor_ERROR,"********************two lane start**************************\n");
        result = IsiRegDefaultsApply_1( pSensorCtx, Sensor_g_aRegDescription_twolane);
		TRACE( Sensor_ERROR,"********************two lane end**************************\n");
	}
...
)
{
    ...
    if (on == BOOL_TRUE)
    {
        ...
        //RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss_1 ( pSensorCtx, Sensor_MODE_SELECT, (RegValue | 0x01U) );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    {   
        ...
        //RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        //result = Sensor_IsiRegWriteIss_1 ( pSensorCtx, Sensor_MODE_SELECT, (RegValue & ~0x01U) );
		result = Sensor_IsiRegWriteIss_1 ( pSensorCtx, Sensor_MODE_SELECT, (RegValue & 0x00U) );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
)
{
	...
	TRACE( Sensor_ERROR, "%s shadow deubg RevId = 0x%08x, value = 0x%08x \n", __FUNCTION__, RevId, value );
...
)
{
    ...
    uint32_t data;
	uint32_t test = 0;//add by shadow 2018/4/11
	uint32_t test_2 = 0;//add by shadow 2018/4/11
...
)
{
	...
	NrOfBytes = 2;
	uint32_t data,data1 = 0;
	result = IsiI2cWriteSensorRegister( handle, address, (uint8_t *)(&value), NrOfBytes, BOOL_TRUE );
	//Sensor_IsiRegReadIss(handle, address,&data);
	//Sensor_IsiRegReadIss(handle, address+1,&data1);
	//TRACE( Sensor_ERROR, "%s (addr 0x%04x=0x%04x 0x%04x=0x%04x)\n", __FUNCTION__, address,data,address+1,data1 );
    return ( result );
}

static RESULT Sensor_IsiRegWriteIss_1
(
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    const uint32_t      value
)
{
	...
	NrOfBytes = 1;
	uint8_t * value_p = (uint8_t *)(&value);
    result = IsiI2cWriteSensorRegister( handle, address,value_p , NrOfBytes, BOOL_TRUE);
	TRACE( Sensor_ERROR, "%s (one: 0x%08x 0x%08x)\n", __FUNCTION__, address, *(value_p));
...
)
{
    ...
    TRACE( Sensor_ERROR, "%s: (------oyyf enter) \n", __FUNCTION__);
    ...
    *pMaxIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    TRACE( Sensor_ERROR, "%s: (------oyyf exit) (\n", __FUNCTION__);
...
)
{
    ...
    usGain = (uint16_t)((NewGain-1) * 16.0f +0.5);//      REG_GAIN=(AG-1)*16

    // write new gain into sensor registers, do not write if nothing has changed
    if( (usGain != pSensorCtx->OldGain) )
    {        
        result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x0046, 0x01);
        result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x003a,usGain );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x0046, 0x00);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        pSensorCtx->OldGain = usGain;
    }

    //return current state
    pSensorCtx->AecCurGain = ( (float)usGain ) / 16.0f +1.0f;
    *pSetGain = pSensorCtx->AecCurGain;
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    if( CoarseIntegrationTime != pSensorCtx->OldCoarseIntegrationTime )//shutter有更�?
    {   
    	result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x0046, 0x01);

        result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x0004, (CoarseIntegrationTime & 0x0000FF00U) >> 8U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x0005, (CoarseIntegrationTime & 0x000000FFU) >> 0U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    *pSetIntegrationTime = pSensorCtx->AecCurIntegrationTime;
	...
	TRACE( Sensor_INFO, "%s:\n"
         ...
         *pSetIntegrationTime,NewIntegrationTime,CoarseIntegrationTime,
         ...
         (CoarseIntegrationTime & 0x000000FFU) >> 0U);
    return ( result );
}
...
)
{
	...
	TRACE( Sensor_INFO, "#%s:pSensorCtx->IsiSensorMipiInfo.ucMipiLanes(%d)\n", __FUNCTION__, pSensorCtx->IsiSensorMipiInfo.ucMipiLanes);
...
)
{
	...
	TRACE( Sensor_ERROR, "#%s: (-----------oyyf) pAfpsInfo->AecMaxIntTime(%f) pSensorCtx->AecMaxIntegrationTime(%f)\n", __FUNCTION__, pAfpsInfo->AecMaxIntTime,pSensorCtx->AecMaxIntegrationTime);
...
#define AFPSCHECKANDADD(_res_) \
	...
	TRACE( Sensor_ERROR, "#%s(%d):pSensorCtx->IsiSensorMipiInfo.ucMipiLanes(%d)\n", __FUNCTION__, __LINE__, pSensorCtx->IsiSensorMipiInfo.ucMipiLanes);
	TRACE( Sensor_ERROR, "#%s(%d):pDummyCtx->IsiSensorMipiInfo.ucMipiLanes(%d)\n", __FUNCTION__, __LINE__, pDummyCtx->IsiSensorMipiInfo.ucMipiLanes);
	...
	switch (pSensorCtx->IsiSensorMipiInfo.ucMipiLanes)
		{
			...
			{
				TRACE( Sensor_ERROR, "SUPPORT_MIPI_ONE_LANE!\n");
				switch(Resolution){
					case ISI_RES_1296_972P30:
					case ISI_RES_1296_972P25:
					case ISI_RES_1296_972P20:
			...
			{
				TRACE( Sensor_ERROR, "SUPPORT_MIPI_TWO_LANE!\n");
				switch(Resolution)
			    {
			        default:
			...
			{
				TRACE( Sensor_ERROR, "SUPPORT_MIPI_FOUR_LANE!\n");
				break;
			}
...
)
{
    ...
    if ((pSensorCtx->VcmInfo.StepMode & 0x0c) != 0) {
 	    vcm_movefull_t = 80* (1<<(pSensorCtx->VcmInfo.StepMode & 0x03)) *1024/((1 << (((pSensorCtx->VcmInfo.StepMode & 0x0c)>>2)-1))*1000);
    }else{
 	    vcm_movefull_t =80*1023/1000;
...
)
{
    ...
    TRACE( Sensor_ERROR, "%s: focus set position_reg_value(%d) position(%d) \n", __FUNCTION__, nPosition, Position);
...
)
{
    ...
    if ( BOOL_TRUE == enable )
    {
        ...
        result = Sensor_IsiRegWriteIss_1( pSensorCtx, 0x5e00, ulRegValue );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    }
    else
    {
...
)
{
    ...
    if ( pIsiSensor != NULL )
    {
		...
		pIsiSensor->pIsiCheckOTPInfo                    = NULL;//shadow
		pIsiSensor->pIsiSetSensorOTPInfo				= NULL;
		pIsiSensor->pIsiEnableSensorOTP					= NULL;
