...
#define  Sensor_NEWEST_TUNING_XML "09-Jun-2015_ZYL-OYYF_IMX274_50013A7_v0.1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "IMX274: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "IMX274: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "IMX274: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "IMX274: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "IMX274: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "IMX274: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x34U                           /**< i2c slave address of the IMX274 camera sensor */
#define Sensor_SLAVE_ADDR2      0x20U
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the IMX274 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "IMX274_MIPI";
extern const IsiRegDescription_t Sensor_g_aRegDescription_fourlane[];
extern const IsiRegDescription_t Sensor_g_fourlane_resolution_3840_2160[];
extern const IsiRegDescription_t Sensor_g_fourlane_resolution_1920_1080[];
			...
			case ISI_RES_TV1080P30: 
            {
                if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						...
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_fourlane_resolution_1920_1080)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set four lane ISI_RES_2104_1560 \n", __FUNCTION__ );
			            }

					}

				}	
				usLineLengthPck = 0x11e; //0x16c; //0x4e0;
			    usFrameLengthLines = 0x20D0; //0xce4; //0xf0a;	
			...
			case ISI_RES_3840_2160P30:
	        {
	            if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						...
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_fourlane_resolution_3840_2160)) != RET_SUCCESS){
							result = RET_FAILURE;
			  	...
			  	usLineLengthPck = 0x210; //0x294; //0x09A0;
		        usFrameLengthLines = 0x11c6; //0x1c70; //0xC30;	
...
)
{
    ...
    pSensorCtx->AecCurIntegrationTime    = 0.001f;
...
)
{
    ...
    result = Sensor_IsiRegWriteIss( pSensorCtx, Sensor_MODE_SELECT, 0x1f );
    if ( result != RET_SUCCESS )
    {
...
)
{
    ...
    if (on == BOOL_TRUE)
    {
        ...
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_MODE_SELECT, 0x00 );//(RegValue & ~0x02U) );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    {   
        ...
        result = Sensor_IsiRegReadIss ( pSensorCtx, Sensor_MODE_SELECT, &RegValue);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_MODE_SELECT, 0x1f);//(RegValue | 0x02U) ); //0x12
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    }
...
)
{
    ...
    *pSetGain = pSensorCtx->AecCurGain;
    ...
    TRACE( Sensor_ERROR, "%s: (exit) gain:%f\n", __FUNCTION__, pSensorCtx->AecCurGain);
...
//    TRACE( Sensor_ERROR, "%s: (enter) %f  (%f -- %f)\n", __FUNCTION__,NewGain,pSensorCtx->AecMinGain,pSensorCtx->AecMaxGain);
    ...
    *pSetGain = pSensorCtx->AecCurGain;
    ...
    TRACE( Sensor_DEBUG, "%s: setgain target(%f) actual(%f) usGain(%d)\n", __FUNCTION__, NewGain, *pSetGain,usGain);
        ...
        for (i=0; i<3; i++) {
            ...
            if (g_suppoted_mipi_lanenum_type & lanes) {
                ...
                while(Sensor_IsiGetCapsIssInternal(&Caps,lanes)==RET_SUCCESS) {
                    ...
                    Caps.Index++;
                    //TRACE(Sensor_ERROR,"zcf test %s",__FUNCTION__);
                }
            }
        }
    }
