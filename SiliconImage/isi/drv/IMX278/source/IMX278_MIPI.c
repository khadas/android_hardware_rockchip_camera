...
#define  Sensor_NEWEST_TUNING_XML "09-Jun-2015_ZYL-OYYF_IMX278_50013A7_v0.1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "IMX278: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "IMX278: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "IMX278: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "IMX278: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "IMX278: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "IMX278: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x34U                           /**< i2c slave address of the IMX278 camera sensor */
#define Sensor_SLAVE_ADDR2      0x20U
#define Sensor_SLAVE_AF_ADDR    0xE4U                           /**< i2c slave address of the IMX278 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "IMX278_MIPI";
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
			case ISI_RES_2104_1560P30:
			{
				...
				usLineLengthPck = 0x26e0;
				if(pConfig->Resolution == ISI_RES_2104_1560P30) {
					usFrameLengthLines = 0x0644;
				}
				osSleep( 10 );
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 1400;
				break;
			}
			...
			case ISI_RES_4208_3120P30:
			{
				if (set2Sensor == BOOL_TRUE) {
					...
					if (pConfig->Resolution == ISI_RES_4208_3120P30) {
						...
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_4208x3120P30_twolane_fpschg);
					} else if (pConfig->Resolution == ISI_RES_4208_3120P20) {
						TRACE( Sensor_DEBUG, "%s: ------ISI_RES_4208_3120P10----- \n", __FUNCTION__ );
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_4208x3120P20_twolane_fpschg);
...
static RESULT IMX278_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,IMX278_read_i2c(handle,address));
...
static int apply_otp(IsiSensorHandle_t   handle,struct otp_struct *otp_ptr)
{
    ...
    // update sensor WB gain
        if (R_gain>0x100) {
            IMX278_write_i2c( handle,0x0210, R_gain>>8);
            IMX278_write_i2c( handle,0x0211, R_gain & 0x00ff);
        }
        if (G_gain>0x100) {
            IMX278_write_i2c( handle,0x020e, G_gain>>8);
            IMX278_write_i2c( handle,0x020f, G_gain & 0x00ff);
            IMX278_write_i2c( handle,0x0214, G_gain>>8);
            IMX278_write_i2c( handle,0x0215, G_gain & 0x00ff);
        }
        if (B_gain>0x100) {
            IMX278_write_i2c( handle,0x0212, B_gain>>8);
            IMX278_write_i2c( handle,0x0213, B_gain & 0x00ff);
        }
    }
...
/*
    ...
    if ( (result != RET_SUCCESS) || (RevId != value) )
    {
        TRACE( Sensor_ERROR, "%s RevId = 0x%08x, value = 0x%08x \n", __FUNCTION__, RevId, value );
        return ( RET_FAILURE );
    }*///check Imx214 and Imx278 isn't ok!
...
)
{
    ...
    TRACE( Sensor_INFO, "%s: enter pAfpsInfo->AecMaxIntTime(%f) pSensorCtx->AecMaxIntegrationTime(%f)\n", __FUNCTION__, pAfpsInfo->AecMaxIntTime,pSensorCtx->AecMaxIntegrationTime);
    ...
    TRACE( Sensor_INFO, "%s: exit pAfpsInfo->AecMaxIntTime(%f) pSensorCtx->AecMaxIntegrationTime(%f)\n", __FUNCTION__, pAfpsInfo->AecMaxIntTime,pSensorCtx->AecMaxIntegrationTime);
