...
#define  Sensor_NEWEST_TUNING_XML "29-07-2017_IMX291_CMK-FLML3-X2A10"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "IMX291: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "IMX291: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "IMX291: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "IMX291: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "IMX291: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "IMX291: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x34U                           /**< i2c slave address of the IMX291 camera sensor */
#define Sensor_SLAVE_ADDR2      0x34U
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the IMX291 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "IMX291_MIPI";

extern const IsiRegDescription_t Sensor_g_aRegDescription_twolane[];
extern const IsiRegDescription_t Sensor_g_1920x1080_twolane[];
...
extern const IsiRegDescription_t Sensor_g_1920x1080_fourlane[];
extern const IsiRegDescription_t Sensor_g_1920x1080P60_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P50_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P30_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P25_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P20_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P15_fourlane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P10_fourlane_fpschg[];
...
)
{
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_TWO_LANE){
    	...
    	switch ( pConfig->Resolution )
        {
			...
			case ISI_RES_TV1080P15:
	        {
	            if (set2Sensor == BOOL_TRUE) {                    
                    if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080_twolane)) != RET_SUCCESS){
							result = RET_FAILURE;
			    ...
			    if(pConfig->Resolution == ISI_RES_TV1080P30) {				
					TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P30 \n", __FUNCTION__ );
					usLineLengthPck = 0x0898;
	            	usFrameLengthLines = 0x08ca; 
				}else if(pConfig->Resolution == ISI_RES_TV1080P25) {
					TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P25 \n", __FUNCTION__ );
					usLineLengthPck = 0x0898;
	            	usFrameLengthLines = 0x0a8c;
				}else if(pConfig->Resolution == ISI_RES_TV1080P15) {
					TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P15 \n", __FUNCTION__ );
					usLineLengthPck = 0x0898;
            ...
            default:
            {
                TRACE( Sensor_ERROR, "%s: two lane Resolution(0x%x) not supported\n", __FUNCTION__, pConfig->Resolution);
                return ( RET_NOTSUPP );
            }
    	}
	    ...
	    switch ( pConfig->Resolution )
	    {
			...
			case ISI_RES_TV1080P10:
	        {
	            if (set2Sensor == BOOL_TRUE) {                    
                    if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080_fourlane)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set one lane Sensor_g_1600x1200 \n", __FUNCTION__ );
			            }
					}
                    ...
                    if (pConfig->Resolution == ISI_RES_TV1080P60) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P60_fourlane_fpschg);
					}else if (pConfig->Resolution == ISI_RES_TV1080P50) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P50_fourlane_fpschg);
					}else if (pConfig->Resolution == ISI_RES_TV1080P30) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P30_fourlane_fpschg);
					}else if (pConfig->Resolution == ISI_RES_TV1080P25) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P25_fourlane_fpschg);
					}else if (pConfig->Resolution == ISI_RES_TV1080P20) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P20_fourlane_fpschg);
					}else if (pConfig->Resolution == ISI_RES_TV1080P15) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P15_fourlane_fpschg);
					}else if (pConfig->Resolution == ISI_RES_TV1080P10) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P10_fourlane_fpschg);
					}
                }
	         					
			    if(pConfig->Resolution == ISI_RES_TV1080P60) {				
					TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P60 \n", __FUNCTION__ );
					usLineLengthPck = 0x0898;
	            	usFrameLengthLines = 0x0456;	
				}else if(pConfig->Resolution == ISI_RES_TV1080P50) {
					TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P50 \n", __FUNCTION__ );
					usLineLengthPck = 0x0898;
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    {
        uint8_t NrOfBytes = IsiGetNrDatBytesIss( address, Sensor_g_aRegDescription_fourlane);
        if ( !NrOfBytes )
        {
            NrOfBytes = 1;
        }
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    NrOfBytes = IsiGetNrDatBytesIss( address, Sensor_g_aRegDescription_fourlane);
    if ( !NrOfBytes )
    {
        NrOfBytes = 1;
    }
...
)
{
	...
	if(NewGain >=1 && NewGain < 2.0)
		usGain = (uint16_t)((20*NewGain - 20) + 0.5);
	else if(NewGain >=2 && NewGain < 4.0)
    ...
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3014, (usGain&0xff));
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3021, (CoarseIntegrationTime & 0x0000FF00U)>>8);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3020, (CoarseIntegrationTime & 0x000000FFU));
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
)
{
    ...
    if ( pIsiSensor != NULL )
    {
        ...
        pIsiSensor->pRegisterTable                      = Sensor_g_aRegDescription_fourlane;
