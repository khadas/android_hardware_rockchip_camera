...
#define  Sensor_NEWEST_TUNING_XML "09-Jun-2015_ZYL-OYYF_IMX317_50013A7_v0.1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "IMX317: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "IMX317: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "IMX317: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "IMX317: ", INFO,     1U );

CREATE_TRACER( Sensor_REG_INFO , "IMX317: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "IMX317: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x34U                           /**< i2c slave address of the IMX317 camera sensor */
#define Sensor_SLAVE_ADDR2      0x20U
#ifdef OV4689_AF_ENABLE
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the IMX317 integrated AD5820 */
...
#endif
#define Sensor_MIN_GAIN_STEP   ( 1.0f /2048.0f); /**< min gain step size used by GUI ( 32/(32-7) - 32/(32-6); min. reg value is 6 as of datasheet; depending on actual gain ) */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "IMX317_MIPI";

extern const IsiRegDescription_t Sensor_g_aRegDescription_twolane[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_3840_2160_720Mbps[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_1920_1080_720Mbps[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_3840_2160_576Mbps[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_1920_1080_576Mbps[];

extern const IsiRegDescription_t Sensor_g_fourlane_resolution_3840_2160_720Mbps[];
extern const IsiRegDescription_t Sensor_g_fourlane_resolution_1920_1080_720Mbps[];
...
static uint16_t g_suppoted_mipi_lanenum_type = SUPPORT_MIPI_FOUR_LANE|SUPPORT_MIPI_TWO_LANE;
    ...
    else
    {
        if(mipi_lanes == SUPPORT_MIPI_ONE_LANE){
            ...
            switch (pIsiSensorCaps->Index) 
            {
				case 0:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_3840_2160P10;
                    break;
                }
                case 1:
                {
...
)
{
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    {
	#if defined(TWOLANE_MIPI720MBPS)
            case ISI_RES_TV1080P30:
	        {
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_1920_1080_720Mbps)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_twolane_resolution_1920_1080_720Mbps \n", __FUNCTION__ );
			            }
					}
				}					
	            usLineLengthPck = 0x0276;//0x011e;//hcc
				if (pConfig->Resolution == ISI_RES_TV1080P30) {
					usFrameLengthLines = 0x0f0a;//a0x20d0;//hcc
				}			
	        ...
	        case ISI_RES_3840_2160P10:
	        {
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_3840_2160_720Mbps)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_twolane_resolution_3840_2160_720Mbps \n", __FUNCTION__ );
			            }
					}
				}				
				usLineLengthPck = 0x04e0;//0x0210;//hcc
				if(pConfig->Resolution == ISI_RES_3840_2160P10) {
	            	usFrameLengthLines = 0x16da;//0x11c6; //hcc
				}
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 720;
	            break;
            ...
            case ISI_RES_TV1080P30:
	        {
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
					TRACE( Sensor_ERROR, "%s: before write\n", __FUNCTION__ );
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_1920_1080_576Mbps)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_twolane_resolution_1920_1080_576Mbps \n", __FUNCTION__ );
			            }
			        TRACE( Sensor_ERROR, "%s: after write\n", __FUNCTION__ );
					}
				}					
	            usLineLengthPck = 0x0276;//0x016c;//hcc
				if (pConfig->Resolution == ISI_RES_TV1080P30) {
					usFrameLengthLines = 0x0f0a;//0x0ce4;//hcc
				}			
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 576;//720;//hcc
	            break;	            
	        }			
	        ...
	        case ISI_RES_3840_2160P10:
	        {
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_3840_2160_576Mbps)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_twolane_resolution_3840_2160_576Mbps \n", __FUNCTION__ );
			            }
					}
				}				
				usLineLengthPck = 0x04e0;//0x0294;//hcc
				if(pConfig->Resolution == ISI_RES_3840_2160P10) {
	            	usFrameLengthLines = 0x16da;//0x1c70;//hcc 
				}
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 576;//720;//hcc
	            break;
		...
		switch ( pConfig->Resolution )
	    {
			case ISI_RES_TV1080P30:
			{
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_fourlane_resolution_1920_1080_720Mbps)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_fourlane_resolution_1920_1080_720Mbps \n", __FUNCTION__ );
						}
					}
				}					
				usLineLengthPck = 0x011e;
				if (pConfig->Resolution == ISI_RES_TV1080P30) {
					usFrameLengthLines = 0x20d0;
				}			
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 720;
				break;				
			}			
			...
			case ISI_RES_3840_2160P30:
			{
				if (set2Sensor == BOOL_TRUE) {
					if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_fourlane_resolution_3840_2160_720Mbps)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_fourlane_resolution_3840_2160_720Mbps \n", __FUNCTION__ );
						}
					}
				}				
				usLineLengthPck = 0x0210;
				if(pConfig->Resolution == ISI_RES_3840_2160P10) {
					usFrameLengthLines = 0x11c6; 
				}
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 720;
				break;
    ...
    TRACE( Sensor_DEBUG, "%s AecMaxIntegrationTime:%f(exit): Resolution %dx%d@%dfps  MIPI %dlanes  res_no_chg: %d   rVtPixClkFreq: %f\n", __FUNCTION__,
...
)
{
	...
	pSensorCtx->AecMaxIntegrationTime = ( ((float)(pSensorCtx->FrameLengthLines - 8)) * ((float)(pSensorCtx->LineLengthPck)) + 112) / pSensorCtx->VtPixClkFreq;
    pSensorCtx->AecMinIntegrationTime = 0.00001f;    
...
static RESULT IMX317_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,IMX317_read_i2c(handle,address));
...
static int apply_otp(IsiSensorHandle_t   handle,struct otp_struct *otp_ptr)
{
    ...
    // update sensor WB gain
        if (R_gain>0x100) {
            IMX317_write_i2c( handle,0x0210, R_gain>>8);
            IMX317_write_i2c( handle,0x0211, R_gain & 0x00ff);
        }
        if (G_gain>0x100) {
            IMX317_write_i2c( handle,0x020e, G_gain>>8);
            IMX317_write_i2c( handle,0x020f, G_gain & 0x00ff);
            IMX317_write_i2c( handle,0x0214, G_gain>>8);
            IMX317_write_i2c( handle,0x0215, G_gain & 0x00ff);
        }
        if (B_gain>0x100) {
            IMX317_write_i2c( handle,0x0212, B_gain>>8);
            IMX317_write_i2c( handle,0x0213, B_gain & 0x00ff);
        }
    }
...
static void debug_sensor_config(const IsiSensorConfig_t *pConfig)
{
	TRACE( Sensor_ERROR, "%s: config list start:\n", __FUNCTION__);
	
	TRACE( Sensor_ERROR, "%s: supported bus-width:%0x\n", __FUNCTION__, pConfig->BusWidth);
	TRACE( Sensor_ERROR, "%s: supported operating modes:%0x\n", __FUNCTION__, pConfig->Mode);
	TRACE( Sensor_ERROR, "%s: sample fields:%0x\n", __FUNCTION__, pConfig->FieldSelection);
	TRACE( Sensor_ERROR, "%s: YCSequence:%0x\n", __FUNCTION__, pConfig->YCSequence);
	TRACE( Sensor_ERROR, "%s: Conv422:%0x\n", __FUNCTION__, pConfig->Conv422);
	TRACE( Sensor_ERROR, "%s: BPat:%0x\n", __FUNCTION__, pConfig->BPat);
	TRACE( Sensor_ERROR, "%s: HPol:%0x\n", __FUNCTION__, pConfig->HPol);
	TRACE( Sensor_ERROR, "%s: VPol:%0x\n", __FUNCTION__, pConfig->VPol);
	TRACE( Sensor_ERROR, "%s: Edge:%0x\n", __FUNCTION__, pConfig->Edge);
	TRACE( Sensor_ERROR, "%s: Bls:%0x\n", __FUNCTION__, pConfig->Bls);
	TRACE( Sensor_ERROR, "%s: CConv:%0x\n", __FUNCTION__, pConfig->CConv);
	TRACE( Sensor_ERROR, "%s: Resolution:%0x\n", __FUNCTION__, pConfig->Resolution);
	TRACE( Sensor_ERROR, "%s: MipiMode:%0x\n", __FUNCTION__, pConfig->MipiMode);
	TRACE( Sensor_ERROR, "%s: AfpsResolutions:%0x\n", __FUNCTION__, pConfig->AfpsResolutions);
	TRACE( Sensor_ERROR, "%s: SensorOutputMode:%0x\n", __FUNCTION__, pConfig->SensorOutputMode);
	TRACE( Sensor_ERROR, "%s: Index:%0x\n", __FUNCTION__, pConfig->Index);
	
	TRACE( Sensor_ERROR, "%s: config list end.\n", __FUNCTION__);
}
...
//    result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_SOFTWARE_RST, 0x10U );
//    RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
//    result = Sensor_IsiRegWriteIss( pSensorCtx, Sensor_MODE_SELECT, 0x12 );
//    if ( result != RET_SUCCESS )
//    {
...
)
{
    ...
    if ( Resolution == pSensorCtx->Config.Resolution )
    {
        ...
        *pNumberOfFramesToSkip = 0;
		TRACE( Sensor_ERROR, "%s no need change,resolution is same \n", __FUNCTION__);
    }
    else
    {
        // change resolution
...
#if 0
		Sensor_IsiRegWriteIss ( pSensorCtx, 0x303E, 0x03 );
		osSleep( 7 );
		Sensor_IsiRegWriteIss ( pSensorCtx, 0x30F4, 0x00 );
		Sensor_IsiRegWriteIss ( pSensorCtx, 0x3018, 0xA2 );
#endif		
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    {
        uint8_t NrOfBytes = IsiGetNrDatBytesIss( address, Sensor_g_twolane_resolution_1920_1080_576Mbps);
        if ( !NrOfBytes )
        {
            NrOfBytes = 1;
        }
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    NrOfBytes = IsiGetNrDatBytesIss( address, Sensor_g_twolane_resolution_1920_1080_576Mbps);
    if ( !NrOfBytes )
    {
        NrOfBytes = 1;
    }
...
)
{
	...
	fGain = (2048.0-2048.0/NewGain);
    usGain = (uint16_t)(fGain + 0.5); 
    // write new gain into sensor registers, do not write if nothing has changed
    //if(pSensorCtx->OldGain != usGain)
    if(1)
    {
    #if 1
    	TRACE( Sensor_ERROR, "%s: 111oldGain=%f,usGain=%f)\n", __FUNCTION__,pSensorCtx->OldGain,usGain );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x300b, (usGain>>8)&0x7);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x300a, (usGain&0xff));
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
	#endif
        pSensorCtx->OldGain = usGain;
        TRACE( Sensor_ERROR, "%s: 222oldGain=%f,usGain=%f)\n", __FUNCTION__,pSensorCtx->OldGain,usGain );
    }

    //calculate gain actually set
    //pSensorCtx->AecCurGain = (float)(512.0/(512.0-usGain));
	pSensorCtx->AecCurGain = (float)(2048.0/(2048.0-usGain));
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
	...
	CoarseIntegrationTime = (uint32_t)(((float)pSensorCtx->FrameLengthLines)-(ShutterWidthPck - 112.0f)/((float)pSensorCtx->LineLengthPck));//SHR mode
	//CoarseIntegrationTime = (uint32_t)(((ShutterWidthPck - 112.0f)/((float)pSensorCtx->LineLengthPck)+12.0f)/((float)pSensorCtx->FrameLengthLines)-1);//SVR
    // write new integration time into sensor registers
    // do not write if nothing has changed
    
    //if(CoarseIntegrationTime < 8)
	//	CoarseIntegrationTime = 8;

	#if 0
	if(CoarseIntegrationTime > pSensorCtx->FrameLengthLines - 4 )
		CoarseIntegrationTime =  pSensorCtx->FrameLengthLines - 4;
	#endif
    ...
    {
#if 1	
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x300d, (CoarseIntegrationTime & 0x0000FF00U) >> 8U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x300c, (CoarseIntegrationTime & 0x000000FFU) );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
#endif
	...
	pSensorCtx->AecCurIntegrationTime = ((((float)pSensorCtx->FrameLengthLines)-((float)CoarseIntegrationTime))*((float)pSensorCtx->LineLengthPck)+112.0f)/((float)pSensorCtx->VtPixClkFreq);//SHR mode
	//pSensorCtx->AecCurIntegrationTime = ((((float)pSensorCtx->FrameLengthLines)*((float)CoarseIntegrationTime+1.0f)-12.0f)*((float)pSensorCtx->LineLengthPck)+112.0f)/((float)pSensorCtx->VtPixClkFreq);//SVR
...
)
{
 ...
 // 	result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3111,1);
 //   RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
//	result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3111,0);
//    RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );  
...
)
{
    ...
    TRACE( Sensor_DEBUG, "%s: (enter) pAfpsInfo->AecMaxIntTime(%f) pSensorCtx->AecMaxIntegrationTime(%f)\n", __FUNCTION__, pAfpsInfo->AecMaxIntTime,pSensorCtx->AecMaxIntegrationTime);
    ...
    TRACE( Sensor_DEBUG, "%s: (exit) pAfpsInfo->AecMaxIntTime(%f) pSensorCtx->AecMaxIntegrationTime(%f)\n", __FUNCTION__, pAfpsInfo->AecMaxIntTime,pSensorCtx->AecMaxIntegrationTime);
...
)
{
	...
	TRACE( Sensor_INFO, "#%s:pAfpsInfo->AecMaxIntTime(%f) pSensorCtx->AecMaxIntegrationTime(%f)\n", __FUNCTION__, pAfpsInfo->AecMaxIntTime,pSensorCtx->AecMaxIntegrationTime);
			...
			case SUPPORT_MIPI_TWO_LANE:
			{
				...
				switch(Resolution)
				{
						...
						case ISI_RES_TV1080P30:
						  ...
						  case ISI_RES_3840_2160P10:
							AFPSCHECKANDADD( ISI_RES_3840_2160P10);
							break;
						default:
			...
			case SUPPORT_MIPI_FOUR_LANE:
			{
			    switch(Resolution)
			    {
					case ISI_RES_TV1080P30:
					  ...
					  case ISI_RES_3840_2160P30:
						AFPSCHECKANDADD( ISI_RES_3840_2160P30);
						break;
...
)
{
    ...
    if ( pIsiSensor != NULL )
    {
		...
		pIsiSensor->pRegisterTable                      = Sensor_g_twolane_resolution_1920_1080_576Mbps;
...
static RESULT Sensor_IsiGetSensorI2cInfo(sensor_i2c_info_t** pdata)
{
    ...
    pSensorI2cInfo->soft_reg_value = 0x10;
