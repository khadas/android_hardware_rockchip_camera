...
#include "OV2718_MIPI_priv.h"

#define  Sensor_NEWEST_TUNING_XML "22-May-2014_OUYANG_OV2718_FX288_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "OV2718: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "OV2718: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "OV2718: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "OV2718: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "OV2718: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "OV2718: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x6cU                           /**< i2c slave address of the OV2718 camera sensor */
#define Sensor_SLAVE_ADDR2      0x20U					  // if SID is low, the sensor's I2C address comes from reg.0X3004 which has a default value of 0x6C 
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the OV2718 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "OV2718_MIPI";

extern const IsiRegDescription_t Sensor_g_aRegDescription[];
extern const IsiRegDescription_t Sensor_g_1920x1080_30fps[];
extern const IsiRegDescription_t Sensor_g_1920x1080_20fps[];
extern const IsiRegDescription_t Sensor_g_1920x1080_15fps[];
extern const IsiRegDescription_t Sensor_g_1920x1080_10fps[];
...
int Sensor_get_PCLK( Sensor_Context_t *pSensorCtx, int XVCLK)
{
	...
	//temp1 = OV2718_read_i2c(0x3086) & 0x0f;
	Sensor_IsiRegReadIss(pSensorCtx, 0x3086, &temp1 );
	temp2 = temp1 & 0x0f;
	sp_div = temp2 + 1;
	
	//temp1 = OV2718_read_i2c(0x3084) & 0x0f;
			...
			case ISI_RES_TV1080P10:
	        {
                ...
                if (set2Sensor == BOOL_TRUE) {                    
                    if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_aRegDescription)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_aRegDescription \n", __FUNCTION__ );
			            }
					}
                }
                #endif
                usLineLengthPck = 0x0dde;
                pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 960;
                
                if (pConfig->Resolution == ISI_RES_TV1080P30) {
                    usFrameLengthLines = 0x0466;
                    result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080_30fps);                        
                    TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P30 \n", __FUNCTION__ );
                }else if(pConfig->Resolution == ISI_RES_TV1080P20) {
                    usFrameLengthLines = 0x0699; 
                    result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080_20fps);  
                    TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P20 \n", __FUNCTION__ );
                }else if(pConfig->Resolution == ISI_RES_TV1080P15) {				
                    TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P15 \n", __FUNCTION__ );
                    usFrameLengthLines = 0x08cc; 
                    result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080_15fps);  
                }else if(pConfig->Resolution == ISI_RES_TV1080P10) {
                    TRACE( Sensor_INFO, "%s: ISI_RES_TV1080P10 \n", __FUNCTION__ );
                    usFrameLengthLines = 0x0d32; 
                    result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080_10fps);  
                }
                break;
            }
...
)
{
    ...
    uint16_t usGain = 0, usgain_bak = 0;
    ...
    usGain = (uint16_t)(NewGain * 100.0f+0.5);
    usgain_bak = usGain;
    ...
    if(1)
    {
		...
		if (usGain < 1)
			...
			usGain = 0x40; //HCG 1024
		else if (usGain < 6)
			usGain = 0x41; //HCG 2048
		else if (usGain < 7)
			usGain = 0x42; //HCG 4096
		else
			usGain = 0x43; //HCG 8192
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x30bb, usGain);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );

		dgain = (ggain << 8) / again_table[i - 1];
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x315a, (dgain>>8) & 0xff);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x315b, dgain & 0xff);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
		#endif
        pSensorCtx->OldGain = usgain_bak;
    }

    //calculate gain actually set
    pSensorCtx->AecCurGain = ( (float)usgain_bak ) / 100.0f;
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x30B6, (CoarseIntegrationTime & 0x0000FF00U) >> 8 );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x30B7, (CoarseIntegrationTime & 0x00000FFU) );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    *pSetIntegrationTime = pSensorCtx->AecCurIntegrationTime;
	...
	TRACE( Sensor_INFO, "%s:\n"
         ...
         *pSetIntegrationTime,NewIntegrationTime,CoarseIntegrationTime,
         ...
         (CoarseIntegrationTime & 0x00000FFU) >> 4U);
    return ( result );
}
