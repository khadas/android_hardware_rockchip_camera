...
#define  Sensor_NEWEST_TUNING_XML "v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "OV5693: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "OV5693: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "OV5693: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "OV5693: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "OV5693: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "OV5693: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x20U                           /**< i2c slave address of the OV5693 camera sensor */
#define Sensor_SLAVE_ADDR2      0x6cU
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the OV5693 integrated AD5820 */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "OV5693_MIPI";
...
int Sensor_get_PCLK( Sensor_Context_t *pSensorCtx, int XVCLK)
{
	...
	int pll3_pre_div32x, pll3_mult, pll3_rdiv, pll3_sys_div, pll3_div2x, VCO;
	int pll3_pre_div32x_map[] = { 2, 3, 4, 6};
	int pll3_div2x_map[] = {	2, 2, 4, 5};
	
	Sensor_IsiRegReadIss(pSensorCtx, 0x3098,&temp1);
	temp2 = temp1 & 0x03;
	pll3_pre_div32x = pll3_pre_div32x_map[temp2];
	
	Sensor_IsiRegReadIss(pSensorCtx, 0x3099, &temp1);
	temp2 = temp1 & 0x1f;
	pll3_mult = temp2 + 1;
	
	Sensor_IsiRegReadIss(pSensorCtx, 0x309c, &temp1);
	temp2 = temp1 & 0x01;
	pll3_rdiv = temp2 + 1;
	
	Sensor_IsiRegReadIss(pSensorCtx, 0x309a, &temp1);
	temp2 = temp1 & 0x0f;
	pll3_sys_div = temp2 + 1;
	
	Sensor_IsiRegReadIss(pSensorCtx, 0x309b, &temp1);
	temp2 = temp1 & 0x03;
	pll3_div2x = pll3_div2x_map[temp2];
	
	VCO = XVCLK * 2 / pll3_pre_div32x * pll3_mult * pll3_rdiv;
	sysclk = VCO * 2 * 2 / pll3_sys_div / pll3_div2x;
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
			...
			case ISI_RES_1296_972P10:
	        {
	            ...
	            usLineLengthPck = 0x0a80;
				if (pConfig->Resolution == ISI_RES_1296_972P30) {
	            	usFrameLengthLines = 0x05d0;
				}else if(pConfig->Resolution == ISI_RES_1296_972P25) {
	            	usFrameLengthLines = 0x06f9;
				}else if(pConfig->Resolution == ISI_RES_1296_972P20) {
	            	usFrameLengthLines = 0x08b8;
				}else if(pConfig->Resolution == ISI_RES_1296_972P15) {
	            	usFrameLengthLines = 0x0ba0;
				}else if(pConfig->Resolution == ISI_RES_1296_972P10) {
	            	usFrameLengthLines = 0x1170;
				}
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 640;
	            break;
...
static RESULT OV5693_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,OV5648_read_i2c(handle,address));
    ...
    int index)
{
	...
	// select bank index
	bank = 0xc0 | index;
	sensor_i2c_write_p(context,camsys_fd,0x3d84, bank, i2c_base_info);
	// read otp into buffer
	sensor_i2c_write_p(context,camsys_fd,0x3d81, 0x01, i2c_base_info);
	osSleep(5);
	// read flag
	address = 0x3d00;
	flag = sensor_i2c_read_p(context,camsys_fd,address, i2c_base_info);
	flag = flag & 0xc0;
	// clear otp buffer
	for(i=0;i<16;i++) {
	...
	struct otp_struct *otp_ptr)
{
	...
	osSleep(5);
	address = 0x3d00;
	(*otp_ptr).module_integrator_id = sensor_i2c_read_p(context,camsys_fd,address + 1, i2c_base_info);
	(*otp_ptr).lens_id = sensor_i2c_read_p(context,camsys_fd,address + 2, i2c_base_info);
	(*otp_ptr).production_year = sensor_i2c_read_p(context,camsys_fd,address + 3, i2c_base_info);
	(*otp_ptr).production_month = sensor_i2c_read_p(context,camsys_fd,address + 4, i2c_base_info);
	(*otp_ptr).production_day = sensor_i2c_read_p(context,camsys_fd,address + 5, i2c_base_info);
	temp = sensor_i2c_read_p(context,camsys_fd,address + 10, i2c_base_info);
	(*otp_ptr).rg_ratio = (sensor_i2c_read_p(context,camsys_fd,address + 6, i2c_base_info)<<2) + ((temp>>6) & 0x03);
	(*otp_ptr).bg_ratio = (sensor_i2c_read_p(context,camsys_fd,address + 7, i2c_base_info)<<2) + ((temp>>4) & 0x03);
	(*otp_ptr).light_rg = (sensor_i2c_read_p(context,camsys_fd,address + 8, i2c_base_info) <<2) + ((temp>>2) & 0x03);
	(*otp_ptr).light_bg = (sensor_i2c_read_p(context,camsys_fd,address + 9, i2c_base_info)<<2) + (temp & 0x03);
	(*otp_ptr).user_data[0] = sensor_i2c_read_p(context,camsys_fd,address + 11, i2c_base_info);
	(*otp_ptr).user_data[1] = sensor_i2c_read_p(context,camsys_fd,address + 12, i2c_base_info);
	(*otp_ptr).user_data[2] = sensor_i2c_read_p(context,camsys_fd,address + 13, i2c_base_info);
	(*otp_ptr).user_data[3] = sensor_i2c_read_p(context,camsys_fd,address + 14, i2c_base_info);
	(*otp_ptr).user_data[4] = sensor_i2c_read_p(context,camsys_fd,address + 15, i2c_base_info);
	// clear otp buffer
	for(i=0;i<16;i++) {
		sensor_i2c_write_p(context,camsys_fd,0x3d00 + i, 0x00, i2c_base_info);
	}
	...
	struct otp_struct *otp_ptr)
{
	...
	address = 0x3d01;
	for(i=0;i<15;i++) {
		(* otp_ptr).lenc[i] = sensor_i2c_read_p(context,camsys_fd,address, i2c_base_info);
		address++;
	}
	...
	struct otp_struct * otp_ptr)
{
	...
	// read flag
	address = 0x3d00 + (index-1)*4 + code*2;
	vcm = sensor_i2c_read_p(context,camsys_fd,address, i2c_base_info);
	vcm = (vcm & 0x03) + (sensor_i2c_read_p(context,camsys_fd,(address+1), i2c_base_info)<<2);
	if(code==1) {
		(* otp_ptr).VCM_end = vcm;
	}
	else{
...
{
	if(R_gain>0x400) {
		OV5693_write_i2c(handle,0x3400, R_gain>>8);
		OV5693_write_i2c(handle,0x3401, R_gain & 0x00ff);
	}
	if(G_gain>0x400) {
		OV5693_write_i2c(handle,0x3402, G_gain>>8);
		OV5693_write_i2c(handle,0x3403, G_gain & 0x00ff);
	}
	if(B_gain>0x400) {
		OV5693_write_i2c(handle,0x3404, B_gain>>8);
		OV5693_write_i2c(handle,0x3405, B_gain & 0x00ff);
	}
	return 0;
}
	...
	struct otp_struct * otp_ptr)
{
	...
	temp = OV5693_read_i2c(handle, 0x5000);
	temp = 0x80 | temp;
	OV5693_write_i2c(handle, 0x5000,  temp);
	for(i=0;i<62;i++) {
		OV5693_write_i2c(handle, 0x5800 + i, (*otp_ptr).lenc[i]);
	}
	return 0;
}
int check_read_otp_wb(
	sensor_i2c_write_t*  sensor_i2c_write_p,
    sensor_i2c_read_t*  sensor_i2c_read_p,
    void* context,
    int camsys_fd)
{
	...
	for(i=1;i<=3;i++) {
		temp = check_otp_wb(sensor_i2c_write_p, sensor_i2c_read_p, context, camsys_fd, i);
		if(temp == 2) {
			otp_index = i;
			break;
		}
    ...
    int camsys_fd)
{
	...
	// check first OTP with valid data
	for(i=1;i<=3;i++) {
		temp = check_otp_VCM(sensor_i2c_write_p, sensor_i2c_read_p, context, camsys_fd, i, 0);
		if(temp == 2) {
			otp_index = i;
			break;
		}
	...
	if(i>3) {
		// no valid vcm OTP data
		return 1;
	}
	read_otp_VCM(sensor_i2c_write_p, sensor_i2c_read_p, context, camsys_fd, otp_index, 0, &g_otp_info);
	read_otp_VCM(sensor_i2c_write_p, sensor_i2c_read_p, context, camsys_fd, otp_index, 1, &g_otp_info);

	return 0;
}

int check_read_otp_lenc(
	sensor_i2c_write_t*  sensor_i2c_write_p,
	sensor_i2c_read_t*  sensor_i2c_read_p,
	void* context,
	int camsys_fd)
{
	...
	for(i=1;i<=3;i++) {
		temp = check_otp_lenc(sensor_i2c_write_p, sensor_i2c_read_p, context, camsys_fd, i);
		if(temp == 2) {
			otp_index = i;
			break;
		}
...
)
{
	...
	ret = check_read_otp_lenc(sensor_i2c_write_p,sensor_i2c_read_p,context,camsys_fd);
	ret = check_read_otp_vcm(sensor_i2c_write_p,sensor_i2c_read_p,context,camsys_fd);
	ret = check_read_otp_wb(sensor_i2c_write_p,sensor_i2c_read_p,context,camsys_fd);
	
	sensor_i2c_write_p( context,camsys_fd, Sensor_MODE_SELECT, 0x00, i2c_base_info);
	
	if(ret)
	{
		TRACE( Sensor_ERROR, "%s: module has OTP data!\n", __FUNCTION__ );
		return RET_NOTSUPP;
	}
	else
		return RET_SUCCESS;	
