...
//#define OV7251_AF_ENABLE
 
#define  Sensor_NEWEST_TUNING_XML "22-May-2014_OUYANG_OV7251_FX288_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "OV7251: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "OV7251: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "OV7251: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "OV7251: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "OV7251: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "OV7251: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0xc0U                           /**< i2c slave address of the OV7251 camera sensor */
#define Sensor_SLAVE_ADDR2      0xc0U
#ifdef OV7251_AF_ENABLE
#define Sensor_SLAVE_AF_ADDR    0x00U                           /**< i2c slave address of the OV7251 integrated AD5820 */
#endif
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "OV7251_MIPI";

extern const IsiRegDescription_t Sensor_g_aRegDescription_onelane[];
extern const IsiRegDescription_t Sensor_g_onelane_resolution_640_480[];
extern const IsiRegDescription_t Sensor_g_onelane_resolution_320_240[];
/*
...
static RESULT Sensor_IsiGetColorIss( IsiSensorHandle_t handle, int *pGetColor);
    ...
    else
    {
        if(mipi_lanes == SUPPORT_MIPI_ONE_LANE){
            switch (pIsiSensorCaps->Index) 
            {
                case 0:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_VGAP100;
                    ALOGD("IsiGetCapsIssInternal, pIsiSensorCaps->Resolution = ISI_RES_VGAP100");
                    break;
                }
                /*
                case 1:
...
int Sensor_get_PCLK( Sensor_Context_t *pSensorCtx, int XVCLK)
{
    ...
    int pre_div, Ref_CLK, PLL2_mult, PLL2_div_2x, PLL2_sys_div,VCO;

    Sensor_IsiRegReadIss(  pSensorCtx, 0x3098, &pre_div );
    pre_div = pre_div & 0x1f;
    
    Ref_CLK = XVCLK * 2 / pre_div;

    Sensor_IsiRegReadIss(  pSensorCtx, 0x3099, &PLL2_mult );
    VCO = Ref_CLK*PLL2_mult;
    
    Sensor_IsiRegReadIss(  pSensorCtx, 0x309D, &temp1 );
	if(temp1 & 0x04){
		PLL2_div_2x = 3;
	} else {
		PLL2_div_2x = 2;
    ...
    Sensor_IsiRegReadIss(  pSensorCtx, 0x309A, &temp1 );
   	PLL2_sys_div = (temp1 & 0xf)*2;
    sysclk = VCO * 2/ PLL2_div_2x/ PLL2_sys_div;

	
    TRACE( Sensor_ERROR, "%s pre_div:%d,Ref_CLK:%d,VCO:%d,PLL2_div_2x:%d,PLL2_sys_div:%d, sysclk:%d\n", 
		__FUNCTION__, pre_div,Ref_CLK,VCO,PLL2_div_2x,PLL2_sys_div,sysclk);
    return sysclk;
 }
...
)
{
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
	        ...
	        {
			  	if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_onelane_resolution_640_480)) != RET_SUCCESS){
					result = RET_FAILURE;
					TRACE( Sensor_ERROR, "%s: failed to set one lane ISI_RES_VGAP100 \n", __FUNCTION__ );
	            }else{
	                TRACE( Sensor_ERROR, "%s: success to set one lane ISI_RES_VGAP100 \n", __FUNCTION__ );
	            }

	            usLineLengthPck = 0x03a0; //hts 772x516x100=39835200
	            usFrameLengthLines = 0x0204;//vts

                /* sleep a while, that sensor can take over new default values */
                osSleep( 10 );

				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 800;
	            break;
...
*/
    ...
    //rVtPixClkFreq = usLineLengthPck*usFrameLengthLines*100;
	...
	pSensorCtx->AecMaxIntegrationTime = ( ((float)(pSensorCtx->FrameLengthLines - 20)) * ((float)pSensorCtx->LineLengthPck) ) / pSensorCtx->VtPixClkFreq;
...
)
{
    ...
    pSensorCtx->AecMaxIntegrationTime = ( ((float)(pSensorCtx->FrameLengthLines - 20)) * ((float)pSensorCtx->LineLengthPck) ) / pSensorCtx->VtPixClkFreq;
    pSensorCtx->AecMinIntegrationTime = ((float)pSensorCtx->LineLengthPck) / pSensorCtx->VtPixClkFreq;  
	//pSensorCtx->AecMinIntegrationTime = 0.0001;	
...
static RESULT OV7251_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,OV7251_read_i2c(handle,address));
...
static int update_awb_gain(IsiSensorHandle_t   handle,int R_gain, int G_gain, int B_gain)
{
    ...
    if (R_gain>0x400) {
        OV7251_write_i2c(handle,0x5186, R_gain>>8);
        OV7251_write_i2c(handle,0x5187, R_gain & 0x00ff);
    }
    if (G_gain>0x400) {
        OV7251_write_i2c(handle,0x5188, G_gain>>8);
        OV7251_write_i2c(handle,0x5189, G_gain & 0x00ff);
    }
    if (B_gain>0x400) {
        OV7251_write_i2c(handle,0x518a, B_gain>>8);
        OV7251_write_i2c(handle,0x518b, B_gain & 0x00ff);
    }
    return 0;
}
// call this function after OV7251 initialization
...
)
{
    ...
    if ( Resolution == pSensorCtx->Config.Resolution )
    {
        ...
        *pNumberOfFramesToSkip = 0;
		TRACE( Sensor_ERROR, "%s (Resolution:%x;pSensorCtx->Config.Resolution:%x)  \n", __FUNCTION__,Resolution,pSensorCtx->Config.Resolution);
    }
    else
    {
        // change resolution
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    {
        uint8_t NrOfBytes = IsiGetNrDatBytesIss( address, Sensor_g_aRegDescription_onelane);
        if ( !NrOfBytes )
        {
            NrOfBytes = 1;
        }
...
)
{
  ...
  //  TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
    ...
    NrOfBytes = IsiGetNrDatBytesIss( address, Sensor_g_aRegDescription_onelane);
    if ( !NrOfBytes )
    {
        NrOfBytes = 1;
    }
...
)
{
    ...
    *pGetColor = WHITE_BLACK_SENSOR;//COLOR_SENSOR;
...
)
{
	...
	TRACE( Sensor_DEBUG, "%s: NewGain=%f,AecMinGain=%f,AecMaxGain=%f usGain:%d \n", __FUNCTION__ ,NewGain,pSensorCtx->AecMinGain,pSensorCtx->AecMaxGain,usGain);	
    ...
    *pSetGain = pSensorCtx->AecCurGain;
	Sensor_IsiRegReadIss( pSensorCtx, 0x350b, &reg_gain );

    TRACE( Sensor_DEBUG, "%s: setgain mubiao(%f) shiji(%f) reg_gain:0x%x \n ", __FUNCTION__, NewGain, *pSetGain,reg_gain);
...
)
{
	...
	uint32_t Reg_CoarseIntegrationTime = 0;
	uint16_t temp = 0;
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
	...
	TRACE( Sensor_ERROR, "%s: CoarseIntegrationTime(0x%x) \n", __FUNCTION__, CoarseIntegrationTime);	
    ...
    //return current state
    *pSetIntegrationTime = pSensorCtx->AecCurIntegrationTime;
    TRACE( Sensor_DEBUG, "%s: OldCoarseIntegrationTime:0x%x \n", __FUNCTION__, pSensorCtx->OldCoarseIntegrationTime );

    TRACE( Sensor_DEBUG, "%s: (exit) settime mubiao(%f) shiji(%f) \n", __FUNCTION__, NewIntegrationTime,*pSetIntegrationTime);
	TRACE( Sensor_DEBUG, "%s: \n"
         "pSensorCtx->VtPixClkFreq:%f pSensorCtx->LineLengthPck:%x \n"
         "SetTi=%f    NewTi=%f  CoarseIntegrationTime=0x%x \n"
         "result_intertime = %x  H:%x  M:%x  L:%x \n", __FUNCTION__, 
	...
	Sensor_IsiRegReadIss( pSensorCtx, 0x3500, &temp );
    TRACE( Sensor_DEBUG, "%s: (exit) temp_H(0x%x) \n", __FUNCTION__, temp);	
	Reg_CoarseIntegrationTime = ((uint32_t)temp<<12) & 0x0000F000U;
	Sensor_IsiRegReadIss( pSensorCtx, 0x3501, &temp );
	TRACE( Sensor_DEBUG, "%s: (exit) temp_M(0x%x) \n", __FUNCTION__, temp);		
	Reg_CoarseIntegrationTime |= (((uint32_t)temp<<4) & 0x00000FF0U);
	Sensor_IsiRegReadIss( pSensorCtx, 0x3502, &temp );
    TRACE( Sensor_DEBUG, "%s: (exit) temp_L(0x%x) \n", __FUNCTION__, temp);	
	Reg_CoarseIntegrationTime |= (((uint32_t)temp>>4) & 0x0000000FU);

    TRACE( Sensor_DEBUG, "%s: (exit) Reg_CoarseIntegrationTime(0x%x) \n", __FUNCTION__, Reg_CoarseIntegrationTime);
...
//NewGain =2.0;
    TRACE( Sensor_ERROR, "%s: NewGain=%f, NewIntegrationTime=%f\n", __FUNCTION__, NewGain, NewIntegrationTime );
    ...
    TRACE( Sensor_DEBUG, "%s: set: pSetGain=%f, pSetIntegrationTime=%f, skip=%d \n", __FUNCTION__, *pSetGain, *pSetIntegrationTime, *pNumberOfFramesToSkip );
			...
			case SUPPORT_MIPI_ONE_LANE:
			{
				switch(Resolution)
				{
					default:
						...
						break;
					case ISI_RES_VGAP100:
						AFPSCHECKANDADD(ISI_RES_VGAP100);
						break;
				}
				break;
			}
...
)
{
    ...
    if ( pIsiSensor != NULL )
    {
        ...
        pIsiSensor->pRegisterTable                      = Sensor_g_aRegDescription_onelane;
        ...
        pIsiSensor->pIsiGetColorIss                     = Sensor_IsiGetColorIss;
    }
    else
    {
