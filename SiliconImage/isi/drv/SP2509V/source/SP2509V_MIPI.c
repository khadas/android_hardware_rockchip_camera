...
#define  Sensor_NEWEST_TUNING_XML "SP2509V_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "SP2509V: ", ERROR,    1U );
CREATE_TRACER( Sensor_WARN , "SP2509V: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "SP2509V: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "SP2509V: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "SP2509V: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "SP2509V: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x7aU                           /**< i2c slave address of the SP2509V camera sensor */
#define Sensor_SLAVE_ADDR2      0x3dU
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the SP2509V integrated AD5820 */
#define Sensor_OTP_SLAVE_ADDR   0x6cU
#define MCLK_FREQ	24000000
#define Sensor_MIN_GAIN_STEP   ( 1.0f / 32.0f) /**< min gain step size used by GUI ( 32/(32-7) - 32/(32-6); min. reg value is 6 as of datasheet; depending on actual gain ) */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "SP2509V_MIPI";
...
)
{
    ...
    result = HalSetClock( pSensorCtx->IsiCtx.HalHandle, pSensorCtx->IsiCtx.HalDevID, MCLK_FREQ);
    RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
int Sensor_get_PCLK( Sensor_Context_t *pSensorCtx, int XVCLK)
{
	...
	//temp1 = SP2509V_read_i2c(0x3086) & 0x0f;
	Sensor_IsiRegReadIss(pSensorCtx, 0x3086, &temp1 );
	temp2 = temp1 & 0x0f;
	sp_div = temp2 + 1;
	
	//temp1 = SP2509V_read_i2c(0x3084) & 0x0f;
...
)
{
	...
	if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_ONE_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
			...
			case ISI_RES_1600_1200P10:
	        {
	            ...
	            usLineLengthPck = 0x3b3;//0x0660;
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 600;
				
				if (pConfig->Resolution == ISI_RES_1600_1200P30) {
	            	usFrameLengthLines = 0x04d2;					
	...
	rVtPixClkFreq = 36000000;//Sensor_get_PCLK(pSensorCtx, xclk);
	...
	//pSensorCtx->AecMaxIntegrationTime = 0.1;
    TRACE( Sensor_ERROR, "%s AecMaxIntegrationTime:%f Resolution %dx%d@%dfps MIPI %dlanes  res_no_chg: %d   rVtPixClkFreq: %f\n", __FUNCTION__,
...
static RESULT SP2509V_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,SP2509V_read_i2c(handle,address));
...
static int update_awb_gain(IsiSensorHandle_t   handle,int R_gain, int G_gain, int B_gain)
{
    ...
    if (R_gain>0x400) {
        SP2509V_write_i2c(handle,0x5004, R_gain>>8);
        SP2509V_write_i2c(handle,0x5005, R_gain & 0x00ff);
    }
    if (G_gain>0x400) {
        SP2509V_write_i2c(handle,0x5006, G_gain>>8);
        SP2509V_write_i2c(handle,0x5007, G_gain & 0x00ff);
    }
    if (B_gain>0x400) {
        SP2509V_write_i2c(handle,0x5008, B_gain>>8);
        SP2509V_write_i2c(handle,0x5009, B_gain & 0x00ff);
    }
    return 0;
}
// call this function after SP2509V initialization
...
)
{
    ...
    {
        /* enable streaming */
        result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_PAGE_SELECT, 0x01U);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_MODE_SELECT, 0x01U);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );		
    }
    else
    {   
...
)
{
	...
	result = Sensor_IsiRegWriteIss ( handle, Sensor_PAGE_SELECT, 0 );
	RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    *p_value = 0U;
...
)
{
    ...
    //usGain = (uint16_t)(NewGain / Sensor_MIN_GAIN_STEP + 0.5);
    float gain_step;
	float gain_start;
	uint8_t reg_step;
	uint8_t reg_start;
	if (NewGain < 1.9375) {
		...
		reg_step = 0x01;
		reg_start = 0x10;
	} else if (NewGain < 3.875) {
		gain_step = 0.125;
		gain_start = 2;
		reg_step = 0x02;
		reg_start = 0x20;
	} else if (NewGain < 7.75) {
		gain_step = 0.25;
		gain_start = 4;
		reg_step = 0x04;
		reg_start = 0x40;
	...
	usGain = reg_start + (NewGain - gain_start) / gain_step * reg_step;
	
    // write new gain into sensor registers, do not write if nothing has changed
    if( (usGain != pSensorCtx->OldGain) )
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0xfd, 0x01);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x24, (usGain&0xff));
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );

		result = Sensor_IsiRegWriteIss( pSensorCtx, 0x01, 0x01);
		RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    //pSensorCtx->AecCurGain = ( (float)usGain ) * Sensor_MIN_GAIN_STEP;
    pSensorCtx->AecCurGain = (uint8_t)((NewGain - gain_start) / gain_step) * gain_step + gain_start;

    //return current state
    *pSetGain = pSensorCtx->AecCurGain;

    TRACE( Sensor_DEBUG, "%s: setgain mubiao(%f) shiji(%f), usGain(0x%x)\n", __FUNCTION__, NewGain, *pSetGain, usGain);
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0xfd, 0x01);//page select
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x03, (CoarseIntegrationTime & 0xFF00U) >> 8U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x04, (CoarseIntegrationTime & 0x00FFU));
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
   ...
   // TRACE( Sensor_ERROR, "%s: SetTi=%f NewTi=%f\n", __FUNCTION__, *pSetIntegrationTime,NewIntegrationTime);
	...
	TRACE( Sensor_DEBUG, "%s:\n"
         ...
         *pSetIntegrationTime,NewIntegrationTime,CoarseIntegrationTime,
         result_intertime,
         (CoarseIntegrationTime & 0xFF00U) >> 8U,
         (CoarseIntegrationTime & 0x00FFU));
    return ( result );
}
...
static RESULT Sensor_IsiGetSensorI2cInfo(sensor_i2c_info_t** pdata)
{
    ...
    pSensorI2cInfo->soft_reg_value = Sensor_SOFTWARE_RST_DATA;//0x00;//0x01;       //switch to page 0
