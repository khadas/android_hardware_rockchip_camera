 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "SP5506: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "SP5506: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "SP5506: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "SP5506: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "SP5506: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "SP5506: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x6cU                           /**< i2c slave address of the SP5506 camera sensor */
#define Sensor_SLAVE_ADDR2      0x6cU
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the SP5506 integrated AD5820 */
#define Sensor_OTP_SLAVE_ADDR   0x6cU

#define Sensor_MIN_GAIN_STEP   ( 1.0f / 128.0f); /**< min gain step size used by GUI ( 32/(32-7) - 32/(32-6); min. reg value is 6 as of datasheet; depending on actual gain ) */
#define Sensor_MAX_GAIN_AEC    ( 15.5f )            /**< max. gain used by the AEC (arbitrarily chosen, recommended by Omnivision) */
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "SP5506_MIPI";
...
extern const IsiRegDescription_t Sensor_g_twolane_resolution_1920_1080[];
extern const IsiRegDescription_t Sensor_g_1920x1080P30_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P25_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P20_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P15_twolane_fpschg[];
extern const IsiRegDescription_t Sensor_g_1920x1080P10_twolane_fpschg[];
...
)
{
    ...
    if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_TWO_LANE){
	    ...
	    switch ( pConfig->Resolution )
	    {
	        ...
	        {
				TRACE( Sensor_ERROR, "%s (enter)\n bining pConfig->Resolution:0x%x\n set2Sensor:%d\n res_no_chg:%d", 
...
#if 1
	            		usLineLengthPck = 0x2ee;
				
				if (pConfig->Resolution == ISI_RES_1296_972P30) {
			            	usFrameLengthLines = 0x07d0;
				}else if(pConfig->Resolution == ISI_RES_1296_972P25){
					usFrameLengthLines = 0x0960;
				}else if(pConfig->Resolution == ISI_RES_1296_972P20){
					usFrameLengthLines = 0x0bb8;
				}else if(pConfig->Resolution == ISI_RES_1296_972P15){
					usFrameLengthLines = 0x0fa0;
				}else if(pConfig->Resolution == ISI_RES_1296_972P10){
					usFrameLengthLines = 0x1770;
				}


#else
                Sensor_IsiRegReadIss(  pSensorCtx, 0x380c, &temp1 );
                Sensor_IsiRegReadIss(  pSensorCtx, 0x380d, &temp2 );
                usLineLengthPck = (temp1 & 0xff) << 8 | temp2;

                Sensor_IsiRegReadIss(  pSensorCtx, 0x380e, &temp1 );
                Sensor_IsiRegReadIss(  pSensorCtx, 0x380f, &temp2 );
                usFrameLengthLines = (temp1 & 0x7f) << 8 | temp2;
                TRACE( Sensor_ERROR, "usLineLengthPck: 0x%x, usFrameLengthLines: 0x%x\n", usLineLengthPck, usFrameLengthLines);

#endif
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 450;
	            break;
            ...
            {
                TRACE( Sensor_ERROR, "%s (enter)\n 1080p pConfig->Resolution:%d\n set2Sensor:%d\n res_no_chg:%d", 
                __FUNCTION__,pConfig->Resolution,set2Sensor,res_no_chg);
                if (set2Sensor == BOOL_TRUE) {
                    if (res_no_chg == BOOL_FALSE) {
                        if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_1920_1080)) != RET_SUCCESS){
                            result = RET_FAILURE;
                            TRACE( Sensor_ERROR, "%s: failed to set one lane ISI_RES_1296_972 \n", __FUNCTION__ );
                        }
                    }
                    if (pConfig->Resolution == ISI_RES_TV1080P30) {                        
                            result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P30_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_TV1080P25) {
                            result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P25_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_TV1080P20) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P20_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_TV1080P15) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P15_twolane_fpschg);
                    }else if (pConfig->Resolution == ISI_RES_TV1080P10) {
                        result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1920x1080P10_twolane_fpschg);
                    }
                }
#if 1
                usLineLengthPck = 0x294;
                
                if (pConfig->Resolution == ISI_RES_TV1080P30) {
                    usFrameLengthLines = 0x08e0;
                }else if(pConfig->Resolution == ISI_RES_TV1080P25){
                    usFrameLengthLines = 0x0ac0;
                }else if(pConfig->Resolution == ISI_RES_TV1080P20){
                    usFrameLengthLines = 0x0d70;
                }else if(pConfig->Resolution == ISI_RES_TV1080P15){
                    usFrameLengthLines = 0x11e0;
                }else if(pConfig->Resolution == ISI_RES_TV1080P10){
                    usFrameLengthLines = 0x1ac0;
                }
	        ...
	        case ISI_RES_2592_1944P30:
	        {
				...
				if (pConfig->Resolution == ISI_RES_2592_1944P30) {
					...
					usFrameLengthLines = 0x0980;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P20) {
					usFrameLengthLines = 0x0bd0;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P15) {
					usFrameLengthLines = 0x0fc0;
				}else if(pConfig->Resolution == ISI_RES_2592_1944P10) {
					usFrameLengthLines = 0x1790;
				}
#else
                Sensor_IsiRegReadIss(  pSensorCtx, 0x380c, &temp1 );
                Sensor_IsiRegReadIss(  pSensorCtx, 0x380d, &temp2 );
...
static RESULT SP5506_write_i2c(    
    IsiSensorHandle_t   handle,
    const uint32_t      address,
    ...
    //TRACE( Sensor_ERROR, "%s OTP register (0x%x,0x%x )erro!\n", __FUNCTION__,address,SP5506_read_i2c(handle,address));
...
{
    int otp_flag, addr, temp, i, temp1;
    ...
    //set 0x5001 to ¡°0x02¡±
    temp1 = sensor_i2c_read_p(context,camsys_fd,0x5001, i2c_base_info);//OV5675_read_i2c(0x5001);
    sensor_i2c_write_p(context,camsys_fd,0x5001, 0x02, i2c_base_info);//5675_write_i2c(0x5001, 0x02);
    // read OTP into buffer
    sensor_i2c_write_p(context,camsys_fd,0x3d84, 0xC0, i2c_base_info);//5675_write_i2c(0x3d84, 0xC0);
    //5675_write_i2c(0x3d88, 0x70); // OTP start address
    sensor_i2c_write_p(context,camsys_fd,0x3d89, 0x10, i2c_base_info);//5675_write_i2c(0x3d89, 0x10);
    sensor_i2c_write_p(context,camsys_fd,0x3d8A, 0x70, i2c_base_info);//5675_write_i2c(0x3d8A, 0x70); // OTP end address
    sensor_i2c_write_p(context,camsys_fd,0x3d8B, 0x29, i2c_base_info);//5675_write_i2c(0x3d8B, 0x29);
    sensor_i2c_write_p(context,camsys_fd,0x3d81, 0x01, i2c_base_info);//5675_write_i2c(0x3d81, 0x01); // load otp into buffer
    osSleep(5);

    // OTP into
    otp_flag = sensor_i2c_read_p(context,camsys_fd,0x7010, i2c_base_info);//OV5675_read_i2c(0x7010);
    addr = 0;
    ...
    if(addr != 0) {
        ...
        (*otp_ptr).module_integrator_id = sensor_i2c_read_p(context,camsys_fd,addr, i2c_base_info);//OV5675_read_i2c( addr );
        (*otp_ptr).lens_id = sensor_i2c_read_p(context,camsys_fd,addr + 1, i2c_base_info);//OV5675_read_i2c( addr + 1);
        (*otp_ptr).production_year = sensor_i2c_read_p(context,camsys_fd,addr + 2, i2c_base_info);//OV5675_read_i2c( addr + 2);
        (*otp_ptr).production_month = sensor_i2c_read_p(context,camsys_fd,addr + 3, i2c_base_info);//OV5675_read_i2c( addr + 3);
        (*otp_ptr).production_day = sensor_i2c_read_p(context,camsys_fd,addr + 4, i2c_base_info);//OV5675_read_i2c( addr + 4);
    } else {
    ...
    // OTP WB Calibration
    otp_flag = sensor_i2c_read_p(context,camsys_fd,0x7020, i2c_base_info);//OV5675_read_i2c(0x7020);
    addr = 0;
    if((otp_flag & 0xc0) == 0x40) {
    ...
    if(addr != 0) {
        (*otp_ptr).flag |= 0x40;
        temp = sensor_i2c_read_p(context,camsys_fd,addr + 2, i2c_base_info);//OV5675_read_i2c( addr + 2);
        (*otp_ptr).rg_ratio = (sensor_i2c_read_p(context,camsys_fd,addr, i2c_base_info) << 2) + ((temp>>6) & 0x03);//(OV5675_read_i2c(addr)<<2) + ((temp>>6) & 0x03);
        (*otp_ptr).bg_ratio = (sensor_i2c_read_p(context,camsys_fd,addr + 1, i2c_base_info) << 2) + ((temp>>4) & 0x03);//(OV5675_read_i2c( addr + 1)<<2) + ((temp>>4) & 0x03);
    } else {
        (*otp_ptr).rg_ratio = 0;
        (*otp_ptr).bg_ratio = 0;
    }
    ...
    for(i=0x7010;i<=0x7029;i++) {
        sensor_i2c_write_p(context,camsys_fd,i, 0x0, i2c_base_info);//OV5675_write_i2c(i,0); // clear OTP buffer, recommended use continuous write to accelarate
    }

    //set 0x5001 to ¡°0x0a¡±
    temp1 = sensor_i2c_read_p(context,camsys_fd,0x5001, i2c_base_info);//OV5675_read_i2c(0x5001);
    sensor_i2c_write_p(context,camsys_fd,0x5001, 0x0a, i2c_base_info);//OV5675_write_i2c(0x5001, 0x0a);
...
{
    read_otp(sensor_i2c_write_p, sensor_i2c_read_p, context, camsys_fd);

    return 0;
}

// call this function after SP5506 initialization
...
static int update_otp(IsiSensorHandle_t   handle)
{
    ...
    if ((*otp_ptr).flag & 0x40) {
        ...
        if (R_gain>0x400) {
            SP5506_write_i2c(handle, 0x5019, R_gain>>8);
            SP5506_write_i2c(handle, 0x501a, R_gain & 0x00ff);
        }

        if (G_gain>0x400) {
            SP5506_write_i2c(handle, 0x501b, G_gain>>8);
            SP5506_write_i2c(handle, 0x501c, G_gain & 0x00ff);
        }

        if (B_gain>0x400) {
            SP5506_write_i2c(handle, 0x501d, B_gain>>8);
            SP5506_write_i2c(handle, 0x501e, B_gain & 0x00ff);
        }
    }
...
)
{
    ...
    if(1)
    {
        usGain = usGain & 0x1fff;
        temp = usGain & 0xff;
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3509, temp);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        temp = usGain >> 8;
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3508, temp);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
    ...
    pSensorCtx->AecCurGain = ( (float)usGain ) / 128.0f;

    //return current state
    *pSetGain = pSensorCtx->AecCurGain;

    TRACE( Sensor_INFO, "%s: xuhf-256 setgain mubiao(%f) shiji(%f), usGain: 0x%x\n", __FUNCTION__, NewGain, *pSetGain, usGain);
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    //The unit of exposure for OV5675(SP5506) is different from
    //Omnivision sensors. The actual Tline is twice that
    //of the register value.
    if( CoarseIntegrationTime != pSensorCtx->OldCoarseIntegrationTime )
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3500, (CoarseIntegrationTime / 2 & 0x0000F000U) >> 12U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3501, (CoarseIntegrationTime / 2 & 0x00000FF0U) >> 4U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
	result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3502, (CoarseIntegrationTime / 2 & 0x0000000FU) << 4U );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
